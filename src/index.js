export default {
	async fetch(request, env, ctx) {
		...
		let totals = [], regularTotals = [], advancedTotals = [];
		...
		let tableRowsContent = '', totalsContent = '';
		totals.forEach((total) => {
			tableRowsContent += `<tr><td>${total.project_name} ${total.advanced ? '<span class="advanced"> - Advanced Project</span>' : ''}</td><td>${total.durationFormatted.hours  || '0'}:${total.durationFormatted.minutes || '00'}:${total.durationFormatted.seconds || '00'}</td></tr>`;
		});
...
off the exact milliseconds recorded. Please contact ${
	...
	async scheduled(event, env, ctx) {
		...
		// send an email to the user with a link to the invoice
		...
		// Worker Mail Channels Integration
		send_request = new Request('https://api.mailchannels.net/tx/v1/send', {
			...
			body: JSON.stringify({
				personalizations: [
					{
						to: [{ email: env.TO_EMAIL, name: env.YOUR_NAME }],
					},
				],
				from: {
