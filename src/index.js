export default {
	async fetch(request, env, ctx) {
		// if the startDate and endDate are not present, return an error
		if (!request.url.includes('startDate') || !request.url.includes('endDate')) {
			return new Response(
				JSON.stringify({
					error: 'Invalid date',
				}),
			...
			.then((data) => {
				...
				data.forEach((item) => {
					if (item.duration > 0 && item.workspace_id == env.WORKSPACE_IDENTIFIER) {
						const ADVANCED_TAG_ID = env.ADVANCED_TAG_ID;
						const ADVANCED_RATE = env.ADVANCED_RATE;
						let IS_ADVANCED = false;
						let cost = 0;
						for (let i = 0; i < item.tag_ids.length; i++) {
							if (item.tag_ids[i] == ADVANCED_TAG_ID) {
								IS_ADVANCED = true;
								break;
							}
						}
						if (IS_ADVANCED) {
							cost = parseFloat(ADVANCED_RATE) * item.duration;
						} else {
							cost = parseFloat(env.HOURLY_RATE) * item.duration;
						}
						const id = IS_ADVANCED ? item.id + '-advanced' : item.id;
						timeEntries.push({
							id: id,
		...
		timeEntries.forEach((item) => {
			...
			if (item.advanced) {
				...
				if (index === -1) {
					...
					advancedTotals[index].duration += item.duration;
					advancedTotals[index].cost += item.cost;
				}
			} else {
				let index = regularTotals.findIndex((x) => x.project_id === item.project_id);
				if (index === -1) {
					regularTotals.push({
						project_id: item.project_id,
						duration: item.duration,
					...
					regularTotals[index].duration += item.duration;
					regularTotals[index].cost += item.cost;
				}
			}
		});

		// add regular and advanced totals together
		totals = regularTotals.concat(advancedTotals);
		...
		totals.forEach((total) => {
			tableRowsContent += `<tr><td>${total.project_name} ${
				total.advanced ? '<span class="advanced"> - Advanced Project</span>' : ''
			}</td><td>${total.durationFormatted.hours || '0'}:${total.durationFormatted.minutes || '00'}:${
				total.durationFormatted.seconds || '00'
			}</td></tr>`;
		});
		...
		<section id='totals'> 
			<p>
				<b>Total Hours: </b> 
				<span>
					~${totals.reduce((a, b) => a + b.duration, 0).toFixed(2)}
				</span>
			</p>
			<p>
				<b>Hourly Rate: </b> 
			...
			<p>
				<b>Advanced Rate: </b> 
				<span>
					$${parseFloat(env.ADVANCED_RATE).toFixed(2)}
				</span>
			</p>
			<p>
				<b>Amount Due: </b>
				<span>
					$${(totals.reduce((a, b) => a + b.cost, 0) / 3600).toFixed(2)}
				</span>
			</p>
		</section>`;

		const html = `<!DOCTYPE html><html lang='en'><head> <meta charset='UTF-8'> <meta http-equiv='X-UA-Compatible' content='IE=edge'> <meta name='viewport' content='width=device-width, initial-scale=1.0'> <title>${
			env.YOUR_NAME
...
async function sendEmail(env) {
	...
	const url = env.MAIL_BASE_URL + `/messages?from=Invoices Bot <${env.FROM_EMAIL}>&to=Mackenly Jones <${env.YOUR_EMAIL}>&subject=${subject}&text=${body}`;
	const init = {
		method: 'POST',
		headers: {
	...
	const response = await fetch(url, init).then((response) => response.json()).catch((error) => console.log(error));
